...
annotate!(texas_trend, 1993, -1,
          ...
          Analysis by sjwild.github.io",
          :right, 7));
png(texas_trend, "Images/Texas/texas_synthetic_control_estimates_placebo");
...
#tota lnumber of cases
y = Matrix{Float64}(df[df.Province_State .== "Alberta", 2:(end-1)]) ./ df[df.Province_State .== "Alberta", end] * pop_scale
X = Matrix{Float64}(df[df.Province_State .!= "Alberta", 2:(end-1)]) ./ df[df.Province_State .!= "Alberta", end] * pop_scale
...
y_sd = std(y_pre)
x_sd = [std(X_pre[i, :]) for i in 1:size(X_pre, 1)]

y_pre = (y_pre .- y_mean) #./ y_sd
X_pre = (X_pre .- x_mean) #./ x_sd
y_post = (y_post .- y_mean) #./ y_sd
X_post = (X_post .- x_mean) #./ x_sd
...
y_fit = gq[:, 1:length(y_pre)] .+ y_mean
y_post_est = gq[:, (length(y_pre) + 1):(length(y_pre) + length(y_post))] .+ y_mean
y_fit = exp.(y_fit)
y_post_est = exp.(y_post_est)
AB_m, AB_ll, AB_ul = get_qs(y_fit, y_post_est)

y_actual = Matrix{Float64}(df[df.Province_State .== "Alberta", 2:(end-1)]) ./ df[df.Province_State .== "Alberta", end] * pop_scale


date_list = [(start_date):Day(1):end_date;]


AB_trend = plot_base("Synthetic control estimates: Directly modelling\ncumulative cases",
                     "Num cases (per 100k)",
                     "Date")
plot!(AB_trend, date_list, AB_m, 
      ribbon = (AB_m - AB_ll, AB_ul - AB_m),
...
annotate!(AB_trend, Date(2021, 07, 03), 45, 
          StatsPlots.text("Most restrictions dropped",
          10, :left))
#annotate!(AB_trend, Date(2021, 09, 30), 750,
#          StatsPlots.text("Source: Government of Canada and CDC
#          Analysis by sjwild.github.io",
#          :right, 7))
png(AB_trend, "Images/Alberta/cumulative_cases")
...
AB_trend = plot_base("Synthetic control estimates: total cases direct model",
                     "Num cases (per 100k)",
                     "Date";
                     legend = :bottomleft)
plot!(AB_trend, date_list, AB_m, 
