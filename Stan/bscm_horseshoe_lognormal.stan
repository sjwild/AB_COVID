...
model{
  ...
  tau ~ student_t(nu_g, 0, scale_g);
  caux ~ inv_gamma(0.5 * slab_df, 0.5 * slab_df);
  
  alpha ~ normal(0, 1);

  // model
  y_pre ~ lognormal(mu, sigma);
...
generated quantities{
  ...
  for(f in 1:N_pre){
    y_fit[f] = lognormal_rng(alpha + X_pre[f,] * beta, sigma);
  }

  for(i in 1:N_post){
    y_post[i] = lognormal_rng(alpha + X_post[i,] * beta, sigma);
  }
  
  for (t in 1:N_pre){
    log_lik[t] = lognormal_lpdf(y_pre[t] | y_fit[t], sigma);
  }
